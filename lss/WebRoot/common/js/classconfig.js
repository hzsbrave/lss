	var $table = $('#table');
	 var operateEvents = {
			'click .like': function (e, value, row, index) {
				...
				modal("#classModal",'comfirm');
				var html = "";
			  html += '<form class="form-horizontal" role="form">';
			  html+= '<div class="form-group" style="display:none;">';
			  html+= '   <label class="col-sm-3 control-label" for="major">id：</label>';
			  html+= '   <div class="col-sm-5">';
			  html+= '      <input class="form-control" id="class" type="text" value='+info.id+'>';
			  html+= '    </div>';
			  html+= ' </div> ';
              html += '     <div class="form-group">';
              html += '        <label class="col-sm-3 control-label" for="classname">班级名称：</label>';
              html += '        <div class="col-sm-5">';
              html += '           <input class="form-control" id="className" type="text"  value='+info.className+'>';
              html += '        </div>';
              ...
              html += '     </div>';
              html += '     <div class="form-group">';
              html += '        <label class="col-sm-3 control-label" for="major">专业：</label>';
              html += '        <div class="col-sm-5">';
              ...
              html += '     </div>';
              html += '     <div class="form-group">';
			  html += '        <label class="col-sm-3 control-label" for="classname">学生人数：</label>';
              html += '        <div class="col-sm-5">';
              html += '           <input class="form-control" id="stuCount" type="text"  value='+info.stuCount+'>';
              html += '        </div>';
              html += '        <div class="col-sm-3 alert alert-danger"></div>';
              html += '     </div>';
              html += '  </form>';
	...
	$("#table").bootstrapTable({   
           ...
           url:$('#hidden').val()+"/class/get.action",//数据源/graduationProject/data/data1.json
...
$(function () {
	...
	$refresh.click(function(){
         $table.bootstrapTable('refresh', {url: $('#hidden').val()+'/class/get.action'});
	});
    ...
    $('#comfirm').click(function () {
      	 ...
      	 if($('#comfirm').hasClass("btn-info")){
      		 ...
      		 var url = '&id='+id+'&className='+className+'&majorId='+majorId+'&grade='+grade+'&stuCount='+stuCount;
      		 $.ajax({
   	        	url:$('#hidden').val()+"/class/update.action",
   	        	type:"post",
   	        	data:url,
   	        	dataType:"json",
   	        ...
   	        $.ajax({
   	        	url:$('#hidden').val()+"/class/deleteList.action",
   	        	type:"post",
   	      		traditional:true,
   	        	data:{stuIds:ids},
   	        	dataType:"json",
   	        	success:function (data) {
   	        		 if(data>0){
   					  	alert("恭喜您 ，删除成功，共删除了"+data+"条记录，若无法删除的，则该班级下属有未删除的班级课程信息，不允许删除！");
   					  	$table.bootstrapTable('refresh', {url: $('#hidden').val()+'/class/get.action'});
   					  	//location.reload();
   					  	$(".modal-backdrop").hide();
   					  }else{
   					  	alert("对不起 ，删除失败，该班级下属有未删除的班级课程信息！");								  
   					  }
   	        	},
   	        	error: function (XMLHttpRequest, textStatus, errorThrown) {
