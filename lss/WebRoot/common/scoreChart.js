  ...
  function chart(option,selectIds){
        var arr = new Array();
           var statistical = new Array();
            ...
            var curriculumName = $("#curriculumId option:selected").text();
            console.log("学年:"+term+";课程："+curriculumId+";班级名称："+curriculumName);
            $.ajax({
                url:"/scms/score/getScoreJson",
                data:{ids:selectIds,term:term,curriculumId:curriculumId},
                type:"post",
                dataType:"json",
         ...
         /*  $.get('/scms/score/getScoreJson').done(function (data) {
                for(var i = 0; i < data.length; i++){
                    arr[i] = data[i]["classname"];
                    statistical[i] = getLevel(data[i]["scores"]);
                }
